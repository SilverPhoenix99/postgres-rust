...
use ascii_flags::*;
use lexer_buffer::LexerBuffer;
use lexer_error::LexerErrorCode::*;
use lexer_error::{LexerError, LexerErrorCode};
use token::Token;
use token::TokenKind::*;
use LexerErrorCode::Eof;
...
/// Don't derive Default: might become different from GUC flags
...
impl LexerConfig {
    pub fn new(standard_conforming_strings: bool) -> Self {
...
impl<'source> Lexer<'source> {
    ...
    #[inline]
    pub fn new(source: &'source [u8], config: LexerConfig) -> Self {
        Self {
...
) -> Option<i32>
{
    ...
    span.iter()
        .filter(|c| **c != b'_') // ignore underscores
        .map(u8_to_i32)
        .try_fold(0i32, |acc, n|
