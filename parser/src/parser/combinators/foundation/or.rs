...
macro_rules! match_first {
    ...
    ($head:expr , $($tail:expr),+ $(,)?) => {
        $crate::parser::combinators::foundation::parser(|stream| {
...
pub(in crate::parser::combinators) use match_first;
...
pub(in crate::parser::combinators) use match_first_with_state;

/// Returns the first `Ok` result between the 2 parsers.
///
/// This is equivalent to `L | R`.
pub(in crate::parser::combinators) fn or<L, R>(left: L, right: R)
    -> OrCombi<L, R>
where
    L: Combinator,
...
pub(in crate::parser::combinators) struct OrCombi<L, R> {
    left: L,
    right: R,
}
