...
pub(in crate::parser::combinators) use sequence;

/// Returns the result from both parsers, in order, or the first `Err`.
///
/// This is equivalent to `L & R`.
pub(in crate::parser::combinators) fn and<L, R>(left: L, right: R) -> AndCombi<L, R>
where
    L: Combinator,
    R: Combinator
{
...
pub(in crate::parser::combinators) struct AndCombi<L, R> {
    left: L,
    right: R,
}
